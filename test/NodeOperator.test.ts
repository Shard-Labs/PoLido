...
describe('NodeOperator', function () {
    beforeEach(async function () {
        ...
        // deploy poLido mock contract
        // TODO: change later with the real poLido contract.
        ...
        await lidoMockContract.setOperator(
            nodeOperatorRegistryContract.address
        );
    ...
    describe('Node Operator', async function () {
        ...
        it('Success join an operator', async function () {
            ...
            const vp = await nodeOperatorRegistryContract
                .connect(user1)
        ...
        describe('DAO', async function () {
            ...
            it('Fail pause unpasue', async function () {
                await expect(
                    nodeOperatorRegistryContract.connect(user1).pause()
                ).revertedWith('Permission not found');
                await expect(
                    nodeOperatorRegistryContract.connect(user1).unpause()
                ).revertedWith('Permission not found');
            });
            ...
            it('Success DAO setter', async function () {
                ...
                expect(await nodeOperatorRegistryContract.getLido()).eq(
                    address
                );
                expect(
                    await nodeOperatorRegistryContract.getValidatorFactory()
                ).eq(address);
                expect(await nodeOperatorRegistryContract.getStakeManager()).eq(
                    address
                );
            ...
            it('Fail DAO setter', async function () {
                ...
                const version = '1.11.0';
                await expect(
                    nodeOperatorRegistryContract.connect(user1).setLido(address)
                ).revertedWith('Permission not found');
                await expect(
                    nodeOperatorRegistryContract
                        .connect(user1)
        ...
        describe('Slash operator', async function () {
            it('List slashed operators', async function () {
                ...
                let slashedOperators =
                    await nodeOperatorRegistryContract.getIfOperatorsWasSlashed();
                expect(
            ...
            it('success slash operators and getRewardPercentage ', async function () {
                ...
                await nodeOperatorRegistryContract.slashOperators(res);
                expect(
                    await nodeOperatorRegistryContract.getRewardPercentage(
                        user1Address
                    )
                ).eq(90);
                expect(
...
async function newOperator(_id: number, userAddress: string, signer?: Signer) {
    ...
    expect(
        ...
        .withArgs(_id, name, signerPubkey);
