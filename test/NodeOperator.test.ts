...
let lidoMockContract: StMATICMock;
...
describe("NodeOperator", function () {
    beforeEach(async function () {
        ...
        const lidoMockArtifact: Artifact = await hardhat.artifacts.readArtifact("StMATICMock");
        lidoMockContract = (await deployContract(signer, lidoMockArtifact)) as StMATICMock;
        ...
        await polygonERC20Contract.transfer(stakeManagerMockContract.address, ethers.utils.parseEther("10000"));
    ...
    describe("Node Operator", async function () {
        ...
        it("Success join an operator", async function () {
            ...
            let no = await nodeOperatorRegistryContract["getNodeOperator(uint256)"].call(this, 1);
            ...
            no = await nodeOperatorRegistryContract["getNodeOperator(uint256)"].call(this, 1);
        ...
        it("Fail restake an operator", async function () {
            ...
            await expect(nodeOperatorRegistryContract.connect(user2)
                .restake(toEth("10"), false)).to.revertedWith("Operator not found");

            // revert operator not active
            await expect(nodeOperatorRegistryContract.connect(user1)
                .restake(toEth("10"), false)).to.revertedWith("Invalid status");
        });
        ...
        it("Success to exitOperator", async function () {
            ...
            let no = await nodeOperatorRegistryContract["getNodeOperator(uint256)"].call(this, 1);
            await erc721Contract.mint(no.validatorProxy, 1);
            no = await nodeOperatorRegistryContract["getNodeOperator(uint256)"].call(this, 2);
            await erc721Contract.mint(no.validatorProxy, 2);
            no = await nodeOperatorRegistryContract["getNodeOperator(uint256)"].call(this, 3);
            ...
            no = await nodeOperatorRegistryContract["getNodeOperator(address)"]
                .call(this, user1Address);

            await lidoMockContract.claimTokens2LidoMatic(no.validatorShare);
        ...
        it("Fail to exitOperator", async function () {
            ...
            await expect(nodeOperatorRegistryContract.exitOperator(ethers.constants.AddressZero))
                .revertedWith("Caller is not stMATIC contract");

            await expect(lidoMockContract.claimTokens2LidoMatic(ethers.constants.AddressZero))
                .revertedWith("Operator not found");

            const no = await nodeOperatorRegistryContract["getNodeOperator(address)"]
                .call(this, user1Address);
            await expect(lidoMockContract.claimTokens2LidoMatic(no.validatorShare))
                .revertedWith("Invalid status");
        });
        ...
        it("Success remove node operator", async function () {
            ...
            let no = await nodeOperatorRegistryContract["getNodeOperator(uint256)"].call(this, 2);
            await erc721Contract.mint(no.validatorProxy, 2);
            ...
            no = await nodeOperatorRegistryContract["getNodeOperator(address)"].call(this, user1Address);
            await lidoMockContract.claimTokens2LidoMatic(no.validatorShare);

            no = await nodeOperatorRegistryContract["getNodeOperator(address)"].call(this, user2Address);
            await lidoMockContract.claimTokens2LidoMatic(no.validatorShare);
        ...
        it("Success getNodeOperatorState", async function () {
...

            {
                ...
                await polygonERC20Contract.connect(user1).approve(no1.validatorProxy, toEth("100"));
                // stake a node operator
                await nodeOperatorRegistryContract.connect(user1).stake(toEth("50"), toEth("50"));
            }
        ...
        describe("DAO", async function () {
            ...
            it("Fail pause unpasue", async function () {
                await expect(nodeOperatorRegistryContract.connect(user1).togglePause())
                    .revertedWith("unauthorized");
                await expect(nodeOperatorRegistryContract.connect(user1).togglePause())
                    .revertedWith("unauthorized");
            });
        ...
        describe("Slash operator", async function () {
            ...
            it("success slash operators and getRewardPercentage ", async function () {
                ...
                async function checkPercentages (results: Array<Number>) {
                    const ops = await nodeOperatorRegistryContract.getOperatorInfos(true);
                    expect(ops.length, "ops.length != results.length").eq(results.length);
                    for (let i = 0; i < ops.length; i++) {
                        expect(ops[i].rewardPercentage, i + "-rewardPercentage not valid").eq(results[i]);
                    }
                }
            ...
            it("restake when operator was slashed", async function () {
                ...
                const no = await nodeOperatorRegistryContract["getNodeOperator(uint256)"].call(this, 1);
                await polygonERC20Contract.connect(user1).approve(no.validatorProxy, toEth("100"));
            ...
            it("getOperatorInfos", async function () {
                ...
                let res = await nodeOperatorRegistryContract.getOperatorInfos(false);
                ...
                res = await nodeOperatorRegistryContract.getOperatorInfos(false);
    ...
    describe("Validator proxy", async function () {
        it("check validator Proxy", async function () {
            ...
            for (let i = 0; i < users.length; i++) {
                const no = await nodeOperatorRegistryContract["getNodeOperator(uint256)"].call(this, i + 1);
                const validatorProxyArtifact: Artifact = await hardhat.artifacts.readArtifact("ValidatorProxy");
                const vpc1 = (await hardhat.ethers.getContractAt(validatorProxyArtifact.abi, no.validatorProxy)) as ValidatorProxy;
...
async function newOperator (_id: number, userAddress: string, signer?: SignerWithAddress) {
    const { name, signerPubkey } = getValidatorFakeData();
...
async function checkStats (totalNodeOperator: number,
    ...
    expect(stats[6].toNumber(), "totalWaitNodeOperator").equal(totalWaitNodeOperator);
    expect(stats[7].toNumber(), "totalExitNodeOperator").equal(totalExitNodeOperator);
}
